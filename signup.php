...
$(document).ready(function(e) { 
	$(".check").on("keyup", function(){ //check클래스(아이디 입력받는 칸입니다.)에서 입력을 감지
		...
		$.post( //post방식으로 id_check.php에 입력한 id값을 넘김
			...
			function(data){ 
				if(data){ //만약 data값이 전송되면
					self.parent().parent().find("div.idcheck").html(data); //div태그를 찾아 html방식으로 data를 뿌려줍니다.
				}
			}
		);
	});
...
<body>
	<div class="signup-box">
        ...
        <div>
          <img src="Css/img/logo.png" alt="Logo" id = "logo_img">
        </div>
      ...
      <form action="signup_action.php" method="post">
        ...
        <input type="text" name="id" id="id" class="check" placeholder="아이디" />
		    <div class= "idcheck"> 아이디를 입력해주세요. </div>
